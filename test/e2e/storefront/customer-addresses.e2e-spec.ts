...
describe("Storefront customer addresses e2e tests", () => {
  ...
  async function registerCustomer() {
    ...
    return { token: register.body.accessToken as string };
  }

  it("POST /api/storefront/customers/me/addresses", async () => {
  ...
  it("GET /api/storefront/customers/me/addresses", async () => {
    const { token } = await registerCustomer();
  ...
  it("PATCH /api/storefront/customers/me/addresses/:addressId", async () => {
    const { token } = await registerCustomer();
    ...
    const response = await request(app.getHttpServer())
      .patch(`/api/storefront/customers/me/addresses/${created.body.id}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ city: "Lyon" })
      .expect(200);
  ...
  it("DELETE /api/storefront/customers/me/addresses/:addressId", async () => {
    const { token } = await registerCustomer();
    const created = await request(app.getHttpServer())
      .post("/api/storefront/customers/me/addresses")
      .set("Authorization", `Bearer ${token}`)
    ...
    await request(app.getHttpServer())
      .delete(`/api/storefront/customers/me/addresses/${created.body.id}`)
      .set("Authorization", `Bearer ${token}`)
      .expect(200);
  });
});
