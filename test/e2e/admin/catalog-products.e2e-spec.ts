...
describe("Admin catalog products e2e tests", () => {
  ...
  it("POST /api/admin/products creates a product", async () => {
    ...
    const response = await request(app.getHttpServer())
      ...
      .expect(201);
    expect(response.body.slug).toBe("my-product");
  });

  it("GET /api/admin/products lists products", async () => {
    const { tenant, storeId, token } = await bootstrapOwnerAndStore();
  ...
  it("GET /api/admin/products/:productId returns product details", async () => {
    const { tenant, storeId, token } = await bootstrapOwnerAndStore();
  ...
  it("PATCH /api/admin/products/:productId updates a product", async () => {
    const { tenant, storeId, token } = await bootstrapOwnerAndStore();
    const created = await request(app.getHttpServer())
      .post("/api/admin/products")
      .set("Authorization", `Bearer ${token}`)
    ...
    const response = await request(app.getHttpServer())
      ...
      .set("Authorization", `Bearer ${token}`)
      .send({ title: "Updated Product", status: "PUBLISHED" })
      .expect(200);
    expect(response.body.title).toBe("Updated Product");
    expect(response.body.status).toBe("PUBLISHED");
  });

  it("POST /api/admin/products/:productId/variants creates a variant", async () => {
    const { tenant, storeId, token } = await bootstrapOwnerAndStore();
    const created = await request(app.getHttpServer())
      .post("/api/admin/products")
      .set("Authorization", `Bearer ${token}`)
    ...
    const response = await request(app.getHttpServer())
      .post(`/api/admin/products/${created.body.id}/variants`)
      .set("Authorization", `Bearer ${token}`)
      .send({ sku: "SKU-1", title: "Variant 1" })
      .expect(201);
    expect(response.body.sku).toBe("SKU-1");
  });
});
