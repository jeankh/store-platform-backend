...
describe("Admin customers e2e tests", () => {
  ...
  it("GET /api/admin/customers/:customerId", async () => {
    const { token, customerId } = await setup();
    const response = await request(app.getHttpServer())
  ...
  it("PATCH /api/admin/customers/:customerId", async () => {
    const { token, customerId } = await setup();
    const response = await request(app.getHttpServer())
      .patch(`/api/admin/customers/${customerId}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ firstName: "Jane", marketingEmailOptIn: true })
      .expect(200);
    expect(response.body.firstName).toBe("Jane");
    expect(response.body.preferences.marketingEmailOptIn).toBe(true);
  });
});
