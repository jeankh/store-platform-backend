...
describe("Admin store tax config e2e tests", () => {
  ...
  it("GET /api/admin/stores/:storeId/tax-config returns tax config", async () => {
    const { storeId, token } = await bootstrapOwner();
    await request(app.getHttpServer())
      .patch(`/api/admin/stores/${storeId}/tax-config`)
      .set("Authorization", `Bearer ${token}`)
      .send({ countryCode: "FR", taxInclusive: true })
      .expect(200);
    const response = await request(app.getHttpServer())
      .get(`/api/admin/stores/${storeId}/tax-config`)
      .set("Authorization", `Bearer ${token}`)
      .expect(200);
    expect(response.body.countryCode).toBe("FR");
  });
  it("PATCH /api/admin/stores/:storeId/tax-config updates tax config", async () => {
    const { storeId, token } = await bootstrapOwner();
    const response = await request(app.getHttpServer())
      .patch(`/api/admin/stores/${storeId}/tax-config`)
      .set("Authorization", `Bearer ${token}`)
    ...
    expect(response.body.taxInclusive).toBe(false);
  });
});
